//
//  MKBXSSlotConfigDataModel.swift
//  MKBXPSSwiftProject
//
...
public final class MKBXSSlotConfigDataModel: MKBXSSlotDataBaseModel, @unchecked Sendable {

    private let readQueue = DispatchQueue(label: "slotParamsQueue")
    private let semaphore = DispatchSemaphore(value: 0)
    private var slotTypeList: [String] = []
    ...
    public override func read(sucBlock: @escaping () -> Void, failedBlock: @escaping (Error) -> Void) {
        readQueue.async { [weak self] in
            ...
            guard self.readSlotTypeList() else {
                self.operationFailed(msg: "Read Slot Type List Error", block: failedBlock)
                return
            }
    ...
    public override func config(sucBlock: @escaping () -> Void, failedBlock: @escaping (Error) -> Void) {
        readQueue.async { [weak self] in
            ...
            if self.slotType == .null && !self.canConfigNoData() {
                self.operationFailed(msg: "At least one channel must have valid data", block: failedBlock)
                return
            }
            let success: Bool
    ...
    private func readSlotTypeList() -> Bool {
        ...
        MKBXSInterface.readSlotType(sucBlock: { data in
            success = true
            self.slotTypeList = (data["slotList"] as? [String]) ?? []
    ...
    private func canConfigNoData() -> Bool {
        guard slotTypeList.count >= 3 else { return false }
        var otherNoDataCount = 0
        for (i, type) in slotTypeList.enumerated() where i != index {
            if type == "ff" { otherNoDataCount += 1 }
        }
        return otherNoDataCount < 2
    }
}
